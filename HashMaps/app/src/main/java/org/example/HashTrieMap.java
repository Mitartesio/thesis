package org.example;// HashTrieMap, lock-free concurrent hash tries, design by Prokopec,
...
public class HashTrieMap<K, V> implements OurMap<K, V> {
     ...
     *
     * @param width the Trie width in power-of-two exponents. Values are expected
    ...
    private Result<V> ilookup(final INode i, final int hashcode, final K k,
        ...
        if (main instanceof CNode) {
            @SuppressWarnings("unchecked") final CNode<K, V> cn = (CNode<K, V>) main;
            ...
            if (an instanceof SNode) {
                ...
                @SuppressWarnings("unchecked") final SNode<K, V> sn = (SNode<K, V>) an;
                if (sn.hash() == hashcode) {
                    final V v = sn.get(k);
                    if (null != v) {
    ...
    private Result<SNode<K, V>> ipickupFirst(final BranchNode bn, final int level, final INode parent) {
        ...
        if (bn instanceof SNode) {
            // Found the SNode
            @SuppressWarnings("unchecked") final SNode<K, V> sn = (SNode<K, V>) bn;
            return new Result<SNode<K, V>>(ResultType.FOUND, sn);
        }
    ...
    private void cleanParent(final INode parent, final INode i, final int hashCode, final int level) {
        while (true) {
            ...
            if (pm instanceof CNode) {
                @SuppressWarnings("unchecked") final CNode<K, V> pcn = (CNode<K, V>) pm;
                ...
                if (m instanceof TNode) {
                    @SuppressWarnings("unchecked") final SNode<K, V> untombed = ((TNode<K, V>) m).untombed();
    ...
    private void clean(final INode i, final int level) {
        ...
        if (m instanceof CNode) {
            @SuppressWarnings("unchecked") final CNode<K, V> cn = (CNode<K, V>) m;
    ...
    private TNode<K, V> getTombNode(final BranchNode an) {
        if (an instanceof INode) {
            ...
            if (mn instanceof TNode) {
                @SuppressWarnings("unchecked") final TNode<K, V> tn = (TNode<K, V>) mn;
                return tn;
            }
        }
        return null;
     ...
     *
     * @param hc     the hashcode value
     * @param level  the level (in bit progression)
     * @param bitmap the current {@link CNode}'s bitmap.
     * @param w      the fan width (in bits)
    ...
    static class CNode<K, V> implements MainNode {
         ...
         *
         * @param flagPos a {@link FlagPos} instance
         ...
         * another one.
         *
         * @param position an integer position
         ...
         *
         * @param sNode a {@link SNode} instance
         * @param width the width (in power-of-two exponents)
         */
         ...
         *
         * @param sn1   a first {@link SNode} instance
         * @param sn2   a second {@link SNode} instance
         * @param level the current level (in bit progression)
         * @param width the width (in power-of-two exponents)
         */
         ...
         * computed bitmap.
         *
         * @param array  the {@link BranchNode} array
    ...
    static class SingletonSNode<K, V> extends KeyValueNode<K, V> implements SNode<K, V> {
        ...
        public SNode<K, V> put(final K k, final V v) {
            if (this.key.equals(k)) {
                ...
                @SuppressWarnings("unchecked") final KeyValueNode<K, V>[] array = new KeyValueNode[]{
    ...
    static class MultiSNode<K, V> extends BaseMultiNode<K, V> implements SNode<K, V> {
        ...
        public SNode<K, V> put(final K k, final V v) {
            ...
            if (index >= 0) {
                @SuppressWarnings("unchecked") final KeyValueNode<K, V>[] ar = HashTrieMap.updated(
                        this.content,
                ...
                @SuppressWarnings("unchecked") final KeyValueNode<K, V>[] ar = HashTrieMap.inserted(
                        this.content,
            ...
            for (int i = 0; i < this.content.length; i++) {
                ...
                if (n.key.equals(k)) {
                    if (2 == this.content.length) {
                        ...
                        @SuppressWarnings("unchecked") final KeyValueNode<K, V>[] narr = HashTrieMap.removed(
                                this.content,
                                new KeyValueNode[this.content.length - 1],
                                i);
