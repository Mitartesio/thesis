...
srun --nodes=1 --ntasks=1 --nodelist=${nodes[1]} bash -c '
    ...
    cd "$PROJECT_COPY"
    python3 scripts/new_version.py CupTest WrongStripedMap5Bad5
' &
srun --nodes=1 --ntasks=1 --nodelist=${nodes[2]} bash -c '
    ...
    # Move to the project copy and run Python
    cd "$PROJECT_COPY"
    python3 scripts/new_version.py CupTest WrongStripedMap6Bad5
' &
srun --nodes=1 --ntasks=1 --nodelist=${nodes[3]} bash -c '
    ...
    # Move to the project copy and run Python
    cd "$PROJECT_COPY"
    python3 scripts/new_version.py CupTest WrongStripedMap7Bad5
' &
